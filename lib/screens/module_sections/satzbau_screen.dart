...
class SatzbauScreen extends StatefulWidget {
  ...
  const SatzbauScreen({super.key, required this.moduleCode, required this.title, required this.xpReward});
...
class _SatzbauScreenState extends State<SatzbauScreen> {
  ...
  late final List<SentencePattern> _qs = SentenceEngine.I.generatePatternSet(moduleCode: widget.moduleCode, level: 2, count: 10);
  int _i = 0;
  int? _selected;
  bool _checked = false;
  int _correct = 0;
  ...
  Future<void> _nextOrFinish() async {
    ...
    await _p.completeSection(moduleCode: widget.moduleCode, sectionKey: 'sentence', xpReward: widget.xpReward);
    if (!mounted) return;
    await showDialog(
      context: context,
      builder: (c) => AlertDialog(
        title: const Text('Satzbau tamamlandı ✅'),
        content: Text('Doğru: $_correct / ${_qs.length}\n\nŞimdi dinleme bölümünde aynı yapıyı duyacaksın.'),
  ...
  Widget build(BuildContext context) {
    if (_qs.isEmpty) return const Scaffold(body: Center(child: Text('Satzbau içerik bulunamadı.')));
    final total = _qs.length;
    final progress = ((_i + 1) / total).clamp(0.0, 1.0);
    return CoursePage(
      title: widget.title,
      subtitle: 'Doğru cümleyi kur',
      ...
      body: ListView(
        ...
        children: [
          ...
          AppCard(
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              const Text('Doğru cümleyi seç', style: TextStyle(fontWeight: FontWeight.w900, fontSize: 20)),
              const SizedBox(height: 8),
              Text(_q.exampleTr, style: TextStyle(color: Colors.white.withOpacity(0.76), fontWeight: FontWeight.w700, fontSize: 15)),
              const SizedBox(height: 12),
              OutlinedButton.icon(onPressed: _play, icon: const Icon(Icons.volume_up_rounded), label: const Text('Almanca cümleyi dinle')),
            ]),
          ),
          const SizedBox(height: 14),
